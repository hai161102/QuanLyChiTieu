...
public class MainActivity extends BaseActivity<ActivityMainBinding> {
    ...
    protected void onLoad() {
        ...
        this.itemEarnPayDayAdapter.setListener(dayMoney ->
                DaySpendingActivity.start(
                        MainActivity.this,
                        DAY_DETAILS_ACTIVITY_REQUEST_CODE,
                        dayMoney.id
                )
        );
    ...
    private void reloadView() {
        ...
        list.sort((a, b) -> Math.toIntExact(b.date - a.date));
        if (list.size() > 0) {
            this.binding.emptyView.setVisibility(View.GONE);
            this.binding.rcvNotes.setVisibility(View.VISIBLE);
        }
    ...
    protected void onActivityResult(int requestCode, int resultCode, @Nullable Intent data) {
        ...
        if (resultCode == RESULT_OK) {
            if (requestCode == ADD_ACTIVITY_REQUEST_CODE
                    || requestCode == DAY_DETAILS_ACTIVITY_REQUEST_CODE) {
                this.reloadView();
            }
        }
    }
